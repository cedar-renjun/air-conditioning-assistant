...
| 段 | 长度 | 值 | 含义 |
| :---: | ----- | ----- | ----- |
| 帧头 | 2 | 0x0A, 0x5A | 帧头特征值，用于识别帧头 |
| 接收者 | 2 | 1 | bit0 表示红外模块<br/>bit1 表示主控 <br/>bit2 表示消息模块<br/>bit3 表示蓝牙4.0模块<br/>bit4 表示Wi-Fi模块<br/>bit 5表示服务器模块<br/>bit 6表示手机模块<br/>bit 7表示人机输出模块<br/>bit 8表示人机输入模块<br/>其它位保留|
| 发送者 | 2 | 1 | 含义同上 |
| 内容长度 | 1 | < 255 | 帧消息的数据长度 |
| 内容 | X |  | 帧消息数据 |
| 校验 | 1 | SUM | 等于<开头><接收者><发送者><内容长度><内容> 值相加的结果|

**注解：**
<br/>1.	协议用malloc和free来分配和释放
<br/>2.	发送者负责分配内存，并填充协议；接收者负责释放内存，如果没有内容接收者注册信息，则消息处理模式负责释放内存
...
| 3 | V1.1 | 2015-10-26 | 增加帧命令定义 | cedar |
