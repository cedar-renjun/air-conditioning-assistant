...
// 此函数作为定时器的回调函数
extern "C" void irRecCallback(void)
{
	// Read if IR Receiver -> SPACE [xmt LED off] or a MARK [xmt LED on]
	// digitalRead() is very slow. Optimisation is possible, but makes the code unportable
  // 读引脚电压
	uint8_t  irdata = IR_GPIO_PIN_READ; // 读引脚 
...
extern "C" void irInit(void)
{
  irRec.enableIRIn();
}


extern "C" bool irDecode(decodeResults *result)
{ 
  if(irRec.decode (&results))
  {   
    result->decode_type = results.decode_type;
    result->address     = results.address    ;
    result->value       = results.value      ;
    result->bits        = results.bits       ;
    result->rawbuf      = (unsigned int*)results.rawbuf;
    result->rawlen      = results.rawlen     ;
    result->overflow    = results.overflow   ;          
    irRec.resume();
